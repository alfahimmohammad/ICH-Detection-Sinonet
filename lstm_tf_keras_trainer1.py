...
#valid_embeds = pd.read_csv(path+'/CNN8882_valid_embeds.csv', index_col=False)
#train_embeds = pd.read_csv(path+'/CNN8882_train_embeds.csv', index_col=False)
...
del model 
model = getLSTM1()
model.load_weights(saved_model_dir+'/LSTM1-011-0.907099.h5')
...
#%%
checkpoint = ModelCheckpoint(saved_model_dir+'/LSTM1-{epoch:03d}-{val_auc:03f}.h5', monitor = 'val_auc', verbose = 0, save_best_only = True,
    save_weights_only = True, mode = 'max')

csv_logger = CSVLogger(saved_model_dir+'/LSTM1-8882_training_history.csv', append=True, separator=',')
"""
...
np.save('roc_curve/fpr_LSTM3.npy', fpr)
np.save('roc_curve/tpr_LSTM3.npy', tpr)
plt.figure()
lw = 2
